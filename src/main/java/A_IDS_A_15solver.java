...
public class A_IDS_A_15solver {
	...
	public ArrayList<Node> solutionSteps;

public A_IDS_A_15solver(String heuristic, int[] board){
		
		UserInterface puzzle = new UserInterface(board);			//class for reading in puzzle from user
		
		Node startState = new Node(puzzle.getPuzzle());		//node contains the start state of puzzle
		
		startState.setDepth(0);
				
		System.out.println("\nStarting A* Search with " + heuristic + " a heuristic....This may take a while\n\n");
		for (int i = 0; i < 16; i++) {
 ...
 */
	public void A_Star(Node startState, String heuristic){
		...
		{
			solutionSteps = start_A_Star.getSolutionPath(solution);	//creates ArrayList of solution path
